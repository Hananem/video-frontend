...
import { getSuggestedVideos } from "../../redux/authSlice";
import DateDisplay from "../../pages/videos/DateDisplay";

const SuggestedVideos = () => {
  const dispatch = useDispatch();
  const { suggestedVideos, loading, error } = useSelector((state) => state.auth); // Access from auth state
  const [currentIndex, setCurrentIndex] = useState(0);

  useEffect(() => {
    dispatch(getSuggestedVideos()); // Fetch suggested videos
  }, [dispatch]);

  // Handle navigation
  const handlePrev = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === 0 ? suggestedVideos.length - 1 : prevIndex - 1
    );
  };

  const handleNext = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === suggestedVideos.length - 1 ? 0 : prevIndex + 1
    );
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4">
      ...
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold text-gray-800">Suggested Videos</h2>
      </div>
      ...
      <div className="relative">
          ...
          <div className="overflow-hidden">
            ...
            <div
              ...
              style={{
                ...
                width: `${suggestedVideos.length * 100}%`,
              }}
            >
        ...
        {!loading && !error && (!suggestedVideos || suggestedVideos.length === 0) && (
          <p className="text-center text-gray-500">No suggested videos available</p>
        )}
